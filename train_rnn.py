...
def define_simple_seg_rnn(setting):
    ...
    if rnn_model == 'gru':
        ...
        rnn = GRU(output_dim = hidden_dim, inner_activation = 'hard_sigmoid', activation = 'sigmoid',
        ...
        rnn = EventAttentionLSTM(hidden_dim = 128, output_dim = hidden_dim, inner_activation='hard_sigmoid', activation='sigmoid', W_regularizer = w_reg, U_regularizer = u_reg, b_regularizer = b_reg, input_length = None, return_sequences = attention)(emd)
    else:
        print "error"
    if attention:
        print "add attention"
...
if __name__ == '__main__':
    ...
    for epoch_round in range(nb_epoch):
        model.fit_generator(sample_generator(datasets[0], setting), datasets[0].size, 
